...
from src.audio import  get_audio_config
...
class FreesoundDatasetys(Dataset):
    def __init__(self, folds,
        ...
        self.images_lst = folds
        # self.targets_lst = []
        self.yupu_path=r'/mnt/group-ai-medical/shared/scusenyang_data/bishe/lanzhou/audio_lanzhou_2015/yupu'
        # for img, trg, fold in zip(*folds_data):
        #     if fold in folds:
        #         self.images_lst.append(img)
        #         self.targets_lst.append(trg)
    ...
    def __getitem__(self, idx):
        ...
        target = self.images_lst.label[idx]
        # image=np.load(os.path.join(self.yupu_path,os.path.basename(os.path.dirname(image_file))+'_'+os.path.basename(image_file)+'.npy'))
        image=read_as_melspectrogram(image_file)
        # print(image.shape)
        # print(np.max(image))
